...
import { GetBrandStatistic, GetOnlineMachineCount, GetStoreListByBrand, GetStoreStatistic } from '../../graphQL/Queries'
...
const StatisticManagement = () => {
    ...
    const [onlineMachineCount, setOnlineMachineCount] = useState(0);
    const { data: onlineData } = useQuery(GetOnlineMachineCount, {
        variables: {
            args: [
                {
                    id: selectedItem.id,
    ...
    useEffect(() => {
        if (onlineData) {
            ...
            setOnlineMachineCount(onlineData.getStore[0].managerGetOnlineMachineCount);
        }
    }, [onlineData]);
        ...
        <Box p={2} display="flex" flexDirection={"column"}>
            ...
            <Box
                ...
                >
                    ...
                    >
                        <StatPercentBox
                            title={`${currencyFormatter(displayStatistic.earning || 0)}`}
                            subtitle={t('earning')}
                            icon={
                                <DonutSmallIcon
                    ...
                    >
                        <StatBoxSplit
                            title={t('exchange_10')}
                            subtitle1={t('amount')}
                            val1={numberFormatter(displayStatistic.exchange10AmountTotal || 0)}
                            subtitle2={t('quantity')}
                            val2={numberFormatter(displayStatistic.exchange10QuantityTotal || 0)}
                            icon={
                                <MonetizationOnIcon
                    ...
                    <Box display={"flex"} alignItems={"center"} justifyContent={"space-between"} mb={"1rem"}>
                        ...
                        <Box display={"flex"} gap={"1rem"}>
                            <CapsuleBox
                                ...
                                content={displayStatistic.machineCount?.toString() || ''}
                                borderColor='#7F669D'
                                textColor={colors.grey[100]}
                            />
                            <CapsuleBox
                                title={t('online_machine_count')}
                                content={onlineMachineCount?.toString() || ''}
                                borderColor='#90a8d0'
                                textColor={colors.grey[100]}
                            />
                        </Box>
