...
const Pagination = ({ QUERY, HANDLE_PAGE_CHANGE, TYPE, ARGS_ID, REFETCH, HANDLE_LOADING_STATE }) => {
    ...
    const handleRefetchCursor = (cursor) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            if (response.edges.length > 0) {
                ...
                // SET ALL CURSORS RETRIEVED FROM THE QUERY
                setAllCursors(response.allCursors);

                // SET REFETCH CURSOR TO THE LAST CURSOR OF THE PREVIOUS PAGE
                // SO WHEN WE REFETCH WE WILL SEND THE LAST CURSOR OF THE PREVIOUS PAGE TO GET THE CURRENT PAGE DATA
